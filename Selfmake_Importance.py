...
class GroupJacobianImportance_accumulate(importance.GroupMagnitudeImportance):
    ...
    def __call__(self, group):
        ...
        for i, (dep, idxs) in enumerate(group):
            idxs.sort() # not sure if this is neccessary; just copy from the GroupHessianImportance
...
class JacobianImportance(GroupJacobianImportance):
    pass



class GroupJacobianImportance_accumulate_seperate(importance.GroupMagnitudeImportance):
    """ For distillation.
    
    Different from GroupJacobianImportance_accumulate, in this one,
    the w and bias term of BN layers are seperated in the Jacobian matrix when calculating score!
    ...
    def __init__(self, 
                 group_reduction:str="mean", 
                 normalizer:str='parameters',  # 'num_parameters'
                 bias=False,
                 target_types:list=[nn.modules.conv._ConvNd, nn.Linear, nn.modules.batchnorm._BatchNorm, nn.modules.LayerNorm]):
        self.group_reduction = group_reduction
        self.normalizer = normalizer
            ...
            if isinstance(layer, (nn.modules.conv._ConvNd, nn.Linear)):
                ...
                if layer.weight.grad is None:
                    ...
                    if hasattr(layer, "transposed") and layer.transposed: # transposed convolution
                        ...
                        if layer_name+ '_out_w' not in self._importance:
                            self._importance[layer_name+ '_out_w'] = local_imp
                        else:
                            self._importance[layer_name+ '_out_w'] += local_imp

                        if hasattr(layer, 'bias') and layer.bias is not None:
                            bias = layer.bias.data.clone() # (o)
                            j_bias = self._jacobian[layer_name+'.bias']  # (n, o)

                            jw = torch.einsum('no,o->no', j_bias, bias)
                            local_imp = torch.einsum('no,no->o', jw, jw)
                            if layer_name+ '_out_b' not in self._importance:
                                self._importance[layer_name+ '_out_b'] = local_imp
                            else:
                                self._importance[layer_name+ '_out_b'] += local_imp                                  
            ...
            if prune_fn in [
                ...
                if layer.weight.grad is not None:
                    if hasattr(layer, "transposed") and layer.transposed: # transposed convolution
                        ...
                        local_imp = self._importance[layer_name+ '_out_w'][idxs]
                    ...
                    if hasattr(layer, 'bias') and layer.bias is not None:
                        local_imp = self._importance[layer_name+ '_out_b'][idxs]   
                        group_imp.append(local_imp)
                        group_idxs.append(root_idxs)      
                ...
                if layer.affine:
                    if layer.weight.grad is not None:
                        ...
                        if hasattr(layer, 'bias') and layer.bias is not None:
                            ...
                            assert(0) # normally bn layer with weight would have bias as well
...
class WHCImportance_numpy(tp.importance.GroupMagnitudeImportance):
    """
    The implementation is based on Numpy, so it would be slow;
    """
    ...
    def __call__(self, group, **kwargs):
        ...
        for i, (dep, idxs) in enumerate(group):
            ...
            if prune_fn in [
                ...
                w = w.cpu().numpy()
                l2_norm = np.linalg.norm(w, 2, 1)
                l2_norm_diag = np.diag(l2_norm)

                similar_matrix = 1- abs(1-distance.cdist(w, w, 'cosine'))
                similar_matrix = np.matmul(l2_norm_diag,np.matmul(similar_matrix, l2_norm_diag))  #the larger, the better
                similar_sum = torch.tensor(np.sum(similar_matrix, axis=0))
            ...
            ####################
                ...
                similar_matrix = 1- abs(1-distance.cdist(w, w, 'cosine'))
                similar_matrix = np.matmul(l2_norm_diag,np.matmul(similar_matrix, l2_norm_diag))  #the larger, the better
                local_imp = torch.tensor(np.sum(similar_matrix, axis=0))
            ...
            # WHCImportance should not care about BatchNorm and LayerNorm
