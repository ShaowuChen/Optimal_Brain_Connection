...
<div align="center">
  <img src="./images/framework4.png" alt="Our OBC Framework" width="50%">
...
\mathcal{S}\left(\{\mathbf{w}_m|m\in\mathbb{G}_g\}\right)\triangleq 
 \sum_{m\in \mathbb{G}_g}\mathcal{S}^{\rm (1)}(\mathbf{w}_m)\triangleq\sum_{m\in \mathbb{G}_g}\mathbf{w}_m^{\top}\left(\mathbf{J}_m^{\top}\mathbf{J}_m\right)\mathbf{w}_m$$
  ...
  - $\mathbf{J}_m^{\top}\mathbf{J}_m$ shows distinct patterns and strong parameter interations:
  <div align="center">
    <img src="./images/JTJ.png" alt="JTJ illustration" width="40%">
    <p style="margin-top: 5px;"> $J_m^TJ_m$ of 16 filters </p>
  </div>

  <p style="margin-top: 8px; font-style: italic;">Our JC outperforms several popular criteria: </p>

  - on `VGG/ResNet/ViT` for `classification`:
    <div style="text-align: center;">
      <img src="./images/jc.png" alt="JTJ illustration" width="100%">
    </div>

  -  on `YoLov7` for `detection`:
  <div style="text-align: center;">
    <img src="./images/yolo.png" alt="yolo" width="90%">
  </div>


  -  on `HQ-CLIP` for `multimodal tasks`:
    <div style="text-align: center;">
      <img src="./images/clip.png" alt="clip" width="100%">
    </div>

  -  on `LLM` `Phi-3-mini` for `NLP`:
  <div style="text-align: center;">
    <img src="./images/LLM.png" alt="LLM" width="80%">
  </div>
...
- `benchmark_HqClip_ImageNet.py`: Compares criteria on `HQ-CLIP` evaluated on `ImageNet`.  Just modify `DATA_ROOT` and conduct 
  ```bash
  python benchmark_HqClip_ImageNet.py
  ```
