...
def get_pruning_idxs(model, example_inputs, train_loader, args, target_flops, data_loader_test):
    ...
    if not 'Jacobian' in args.method: # Only consider my Jacobian method
        ...
        while remain_ops / 1e9 > target_flops:# keep pruning
            ...
            print0(f"step {step_count}: ops {remain_ops/ 1e9} G; target {target_flops} G; Speedup {float(ori_ops) /(remain_ops)}x; paras {remain_parameters / 1e6} M, rate {remain_parameters / orig_parameters * 100}%; time {(end_time-start_time)/60:.2f} mins")
            step_count += 1
                ...
                if 'ompressor' in name and ((isinstance(module, torch.nn.Conv2d) and module.in_channels==module.out_channels) \
                    or (isinstance(module, torch.nn.Linear) and module.in_features==module.out_features)):
                        ...
                        for sub_name in module_names[:-1]:
                            parent_module = getattr(parent_module, sub_name)  # father module
...
def main(args):
    ...
    middle_name = ''
    middle_name += 'SkipLast' if args.resnet_skip_block_last else ''
    ...
    if args.prune:
        ...
        if utils.is_main_process() and args.pruning_idx_resume_path==None:
            ...
            start_time = time.time()
            pruning_record_orig_wo = get_pruning_idxs(model, example_inputs, data_loader, args, args.target_flops, data_loader_test)
            end_time = time.time()
        ...
        pruning_record_orig_wo = torch.load(idxs_path, map_location='cpu')
        assert pruning_record_orig_wo!={}, 'pruning_record_orig_wo is empty'
        
        dist.barrier()
        
        
        # After finding pruning idxs using small batch size (to avoid CUDA out of memory), reuse the normal batch size
        if args.Jacobian_batch_size!=None:
            data_loader = torch.utils.data.DataLoader(
            dataset,
